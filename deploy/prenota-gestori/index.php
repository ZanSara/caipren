...
// *************** DB ASSERTIONS ****************************************
        ...
        foreach($lista as $pren){
            for($i=0; $i<$pren['posti']; $i++, $tot++){
                if($pren['id'] == $last_prenid){
                    echo("<td class='P".$pren['id']."' style='background:".$pren['colore'].";border: yellow 4px solid;'>");
                }else{
                    echo("<td class='P".$pren['id']."' style='background:".$pren['colore'].";'>");
                }
    ...
    <footer>
      <div>
        ...
        <a data-toggle="modal" data-target="#Find_Modal" ><li class="btn btn-primary">Cerca</li></a>
    ...
    <script type="text/javascript">
        ...
        $('#newB_Modal').on('hidden.bs.modal', function (event) {
            ...
            $('#newB_Modal input').prop('readonly', true);
            $('#newB_Modal textarea').prop('readonly', true);

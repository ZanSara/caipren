...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $numClienti = mysqli_fetch_array(mysqli_query($dbhandle, "SELECT COUNT(*) FROM Pernottamenti WHERE gestione = 0 AND stagione = ".$year." AND NOT (
...
<?
    ...
    $listadb = mysqli_query($dbhandle, "SELECT *
                                            FROM Pernottamenti
                                            WHERE (gestione=0 AND stagione = ".$year." AND giorno_inizio < ".$giornoInizio." AND giorno_inizio+durata >".$giornoInizio.")");
    ...
    for($g=$giornoInizio; $g<$giornoFine+1; $g++){
        ...
        $listadb = mysqli_query($dbhandle, "SELECT *
                                            FROM Pernottamenti
                                            WHERE (gestione=0 AND stagione = ".$year." AND giorno_inizio=".$g.")");
        while ($row = mysqli_fetch_array($listadb)) {
            $lista[] = $row; // This appends the NEW bookings to $lista, which is usually NOT empty!
        }
        $tot = 0;
...
?>
    ...
    $datalist = mysqli_query($dbhandle, "SELECT * FROM Pernottamenti WHERE gestione = 0 AND stagione = ".$year." AND NOT (
